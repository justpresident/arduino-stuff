...
double VE = 82;              // ВНИМАНИЕ!!! Объёмный КПД ДВС: обычно в районе 80 (если расход реально меньше - то уменьшаем значение в процентном соотношении), duster 82
...
double FuelDensityGramsPerLiter = 750;   // константа - грамм бензина в 1 литре бензина
//-------------------------------------------------------------------------------------------------------------
...
double rpm_var, uoz_var, raschet_nagruzka_dvigatelya_var, maf_var, polozh_dross_zaslon, uroven_topliva_var, IMAP, MAF, FuelFlowGramsPerSecond,FuelFlowLitersPerSecond,time,
...
void loop()
{
	...
	if (rpm_var>engine_on_rpm) {     // если обороты больше engine_on_rpm то считаем расход и расстояние (машина заведена)
		...
		if (raschet_nagruzka_dvigatelya_var>5.0)  {  // Проверка торможения двигателем по нагрузке двигателя, если информации о нагрузке в ЭБУ нету то поменять на ((fss_val==2) or (speed_var>70))
			...
			MAF=double(IMAP/120.0)*double(VE/100.0)*ED*28.9644/8.314472;
			...
			FuelFlowGramsPerSecond = double(MAF/AirFuelRatio)*ls_term_val;   // Получаем расход грамм бензина в секунду в соотношении 14,7 воздуха/к 1 литра бензина  
		...
		if ((time_new-time_old_gurnal)>5000) {
	...
	if (tmp_var>102) {  // Если температура двигателя выше 102 градусов то вывести предупреждение о высокой температуре на экран и прогудеть бипером
		lcd.clear();
		lcd.setCursor(5,0);
		lcd.print("WARNING!!!");
		lcd.setCursor(0,2);
		lcd.print("VYSOKAYA TEMPERATURA");
		lcd.setCursor(8,3);
		lcd.print(tmp_var);  //Температура    
		lcd.print(" \1");
		lcd.print("C");
